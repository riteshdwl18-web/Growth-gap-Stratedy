...
const screenerDownloadConfirmOpen = ref(false)
const dismissedRunId = ref('')
const runsSectionRef = ref<HTMLElement | null>(null)
const DISMISSED_RUN_STORAGE_KEY = 'screener.dismissedRunId'
...
function scrollToRunsSection(): void {
  runsSectionRef.value?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}

function requestScreenerRunDownload(): void {
  if (!controller.canDownloadCsv) {
    return
  }
  screenerDownloadConfirmOpen.value = true
}

function confirmScreenerRunDownload(): void {
  screenerDownloadConfirmOpen.value = false
...
<template>
    ...
    <v-col cols="12">
      <v-card class="workflow-guide-card mb-4">
        <v-card-title class="card-heading d-flex justify-space-between align-center">
          <div>
            <div>Screener Workflow Guide</div>
        ...
        <v-card-text>
          ...
          <v-sheet v-if="showSelectedRunStatus && controller.selectedRun" class="run-status-strip mb-4" rounded="lg">
            <div class="run-status-strip-header">
              ...
              <div class="d-flex align-center ga-2 flex-wrap">
                <v-btn variant="tonal" size="small" color="primary" @click="scrollToRunsSection">See Runs & Results</v-btn>
                <v-btn
                  variant="tonal"
                  size="small"
                  :disabled="!controller.canDownloadCsv"
                  @click="requestScreenerRunDownload"
                >
                  Download CSV
                </v-btn>
                <v-btn
          ...
          <template v-if="controller.uploadResult?.valid">
            ...
            <div class="mt-2 d-flex ga-2 align-center flex-wrap">
              ...
              <v-btn class="action-btn" variant="tonal" color="primary" @click="scrollToRunsSection">See Runs & Results</v-btn>
  ...
  <v-dialog v-model="screenerDownloadConfirmOpen" max-width="460">
    <v-card>
      <v-card-title>Download Run CSV</v-card-title>
      <v-card-text>Are you sure you want to download this run CSV?</v-card-text>
      <v-card-actions class="justify-end">
        <v-btn variant="text" color="secondary" @click="screenerDownloadConfirmOpen = false">Cancel</v-btn>
        <v-btn variant="flat" color="primary" @click="confirmScreenerRunDownload">Yes, Download</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
