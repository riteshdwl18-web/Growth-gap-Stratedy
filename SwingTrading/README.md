...
Use `--refresh` to force fresh Screener fetches for the current run.

#### Screener fetch mode
ROCE and annual revenue are fetched live from Screener for each run.  
Consolidated data is preferred, with standalone used only when consolidated data is incomplete or unavailable.
