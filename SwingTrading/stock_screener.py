...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--refresh",
        action="store_true",
        help="Fetch fresh Screener data (cache disabled)",
    )
    parser.add_argument(
        "--sheet-env",
        choices=["dev", "prod"],
...
def _prefer_fresher_annual(primary: dict[str, float], fallback: dict[str, float]) -> dict[str, float]:
    """Prefer a complete primary series; use fallback only when it is clearly better."""
    primary_complete = _has_complete_screener_annual(primary)
    fallback_complete = _has_complete_screener_annual(fallback)

    if primary_complete and not fallback_complete:
        return primary
    if fallback_complete and not primary_complete:
        return fallback
    ...
    if f_year > p_year:
        return fallback
    if not primary_complete and f_year == p_year and len(fallback) > len(primary):
        return fallback
    return primary
...
def get_annual_revenue(symbol: str, refresh: bool = False) -> tuple[dict[str, float], str]:
    ...
    No local cache; fetches from Screener on each call. No yfinance fallback.
    """
...
def get_roce_snapshot(ticker: yf.Ticker) -> tuple[float | None, float | None, dict]:
    """Returns (roce_pct, ttm_rev_cr, annual_revenue_dict) from live Screener fetches."""
    return get_roce_and_ttm(ticker)
...
def check_sales_cagr(
    ...
    # Fetch Screener data first so ROCE/TTM and annual revenue use the same source-selection logic.
    ticker = yf.Ticker(symbol)
    symbol_plain = symbol.split(".")[0]
    roce, ttm_revenue, annual_revenue = get_roce_snapshot(ticker)
    market_cap_cr = get_market_cap_cr(ticker)
    result["market_cap_cr"] = round(market_cap_cr, 2) if market_cap_cr is not None else None

    if _has_complete_screener_annual(annual_revenue):
        revenue, currency = dict(sorted(annual_revenue.items())), "INR"
    else:
...
if __name__ == "__main__":
    ...
    print(f"Workers: {WORKERS} | Checkpoint every {CHECKPOINT_EVERY} | Live Screener fetches")
